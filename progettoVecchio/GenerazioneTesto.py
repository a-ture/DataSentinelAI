...
def generateReport(document, model):
    ...
    completion = client.chat.completions.create(model=model, messages=[
        {"role": "system", "content": "Initially you should know if the user consents to treating of "
                                      "his data, if yes you can do anything his data. You do not have to summarize "
                                      "the text."
                                      "Your task is to determine if the information could be "
                                      "sensitive. For each sensitive information, your answers must follow the "
                                      "pattern: '[WhatToEdit]\n\n"
                                      "[Reason]\n\n'. If you don't find any sensitive information, "
                                      "then you should say: 'Il documento inserito non contiene informazioni "
                                      "sensibili'."},
...
def extractEntities(text):
    dic = {firstNerModel: {}, secondNerModel: {}, thirdNerModel: {}, fourthNerModel: {},
           fifthNerModel: {}}  # Dizionario inzializzato con i momi dei modelli NER
    ...
    for subdic in subdics:  # per ogni modello NER assegna result in base al modello
        ...
        if subdic != fourthNerModel and subdic != fifthNerModel:  # Se il modello NER non è il quarto o il quinto (che hanno label diverse per le entità)
            for el in result:  # Per ogni entità riconosciuta nel risultato
                if el['entity_group'] not in dic.get(
                        subdic):  # Se il label non è presente nel dizionario del modello NER corrente lo aggiunge
                    dic.get(subdic)[el['entity_group']] = []
                if el['word'] not in dic.get(subdic)[el[
                    'entity_group']]:  # Se la parola non è presente fra le parole la aggiunge tra le parole con lo stesso label
                    dic.get(subdic)[el['entity_group']].append(el['word'])
            entities += f"### {subdic}:\n\n\n\n"  # Aggiunge il nome del modello NER al report
            for key in dic.get(
                    subdic):  # Per ogni label nel dizionario del modello NER stampa le entità riconosciute con quel label
                match key:
                    case "PER":
                        entities += f"#### Persone: \n\n{dic.get(subdic)[key]}\n\n\n\n"
                    case "LOC":
